<template>
  <div class="pagination clr" v-if="pageCount > 0">
    ...
    <template v-else>
      ...
      <template v-if="temp <= 0 + adjustedCriticalNum - 2">
      ...
      <template v-else-if="temp > 0 + adjustedCriticalNum - 2 && temp < pageCount - adjustedCriticalNum + 1">
        <p @click="flip(0)">1</p>
        <span>...</span>
      ...
      <template v-else-if="temp >= pageCount - adjustedCriticalNum + 1">
        <p @click="flip(0)">1</p>
        <span>...</span>
...
export default {
  ...
  computed: {
    ...
    isLast() {
      return this.temp === this.pageCount - 1;
    },
    pageIndexArray() {
      let _arr = [];
    ...
    middleParagraphPageIndexArray() {
      let startPage = this.temp + 1 - this.flankLength;
      let endPage = this.temp + this.flankLength + 2;
  ...
  methods: {
    ...
    isActive(val) {
      return val === this.temp;
    },
    move(flag) {
      if (flag) {
        if (this.temp < this.pageCount - 1) {
          this.flip(this.temp + 1);
        }
      } else {
        if (this.temp > 0) {
          this.flip(this.temp - 1);
