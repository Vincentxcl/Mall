<template>
  <dialog-window class="fileHolder" v-model="isShowField" :width="width" :height="height" :border="{ top: 3, right: 3, bottom: 3, left: 3 }">
    <template slot="title"> HTML5上传图片(AJAX) </template>
    <template slot="content">
      <!--uploader上传对话框-->
      <div class="view_upload" v-show="fileArray.length == 0">
        <!-- 1 -->
        <btn type="brand" title="选择图片" @click.native="clickInput">
          <icon icon="icon-shangchuantupian"></icon>
          <span>&nbsp;选择图片</span>
        </btn>
        <!-- 2 -->
        <div class="support">
          <p>支持的文件格式：{{ supportContentTypeStringify }}</p>
          <p>支持的文件大小：{{ byteToMega(supportContentSize, 0) }}M</p>
        </div>
        <!-- 3 -->
        <div class="msg">
          {{ tips }}
        ...
        <!-- 4 -->
        <input ref="input" type="file" class="hide" :multiple="multiple" :accept="supportContentTypeStringify" @change="getFiles" />
      </div>
