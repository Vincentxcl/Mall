<template>
	<div class="login">
		...
		<text-box ref="randomWords" v-model="randomWords" placeholder="点击获取验证码" :maxlength="4" @focus="showVCode()" @iconClick="reverseVCodeDisplay()" title="验证码" icon="icon-yanzhengyanzhengma">
			<div class="msg" slot="tips" slot-scope="textbox">
				<p class="fl">{{textbox.tips}}</p>
			</div>
		</text-box>
			...
			submit() {
				if (this.$refs.account.check() && this.$refs.password.check() && this.$refs.randomWords.check()) {
					...
					tryLogin(params)
						.then(res => {
							this.clear();
							this.message = decodeURI(res.data.msg);
							setCookie('user', res.data.user, this.num);
							//返回是否登录成功 true / false
						...
						.catch(error => {
							this.refreshVCode();
							this.clear();
							this.message = decodeURI(error.response.data.msg);
							setCookie('user', error.response.data.user, this.num);
						});
				}
			},
		},
...
<style>
	...
	div.login {
		...
		div.login div.msg {
			...
			div.login div.msg a, div.login div.msg label {
				font-size: 15px;
				color: white;
				cursor: pointer;
			}
